...
#força a auto reconexão entr o Banco de dados
...
#dados de autenticação com a planilha
credentials = ServiceAccountCredentials.from_json_keyfile_name('/home/tecnico/agsolve/temperatureSensors/credentials.json',scope)
gc = gspread.authorize(credentials)
worksheet   = gc.open_by_key('1of7wKCkT0uNHB5xt-AxDF38zFcQYtcpjJS93eODZVdY').get_worksheet(0) #ambos - sec 
worksheet1  = gc.open_by_key('1of7wKCkT0uNHB5xt-AxDF38zFcQYtcpjJS93eODZVdY').get_worksheet(1) #SHT31 - sec
worksheet2  = gc.open_by_key('1of7wKCkT0uNHB5xt-AxDF38zFcQYtcpjJS93eODZVdY').get_worksheet(2) #DS18B20 - sec
worksheetH  = gc.open_by_key('1kRgVvF0q1j1zYeHF7dgaaTnRu7tQxKgXj-5Jw16WWnA').get_worksheet(0) #ambos - h 
worksheetH1 = gc.open_by_key('1kRgVvF0q1j1zYeHF7dgaaTnRu7tQxKgXj-5Jw16WWnA').get_worksheet(1) #SHT31 - h
worksheetH2 = gc.open_by_key('1kRgVvF0q1j1zYeHF7dgaaTnRu7tQxKgXj-5Jw16WWnA').get_worksheet(2) #DS18B20 - h
...
#caminho para achar o endereço I2C
base_dir = '/sys/bus/w1/devices/'
#achar o diretório que começa com 28xxxxx
device_folder = glob.glob(base_dir + '28*')[0]

device_file = device_folder + '/w1_slave'

def read_temp_raw():
  # "cat w1_slave" -> vai pegar a informação que contém dentro do w1,
  #  que é se foi reconhecido e a temperatura
...
def read_temp():
    lines = read_temp_raw()
    while lines[0].strip()[-3:] != 'YES': #se tiver esse "YES" é pq reconheceu o sensor
        time.sleep(0.1)
        lines = read_temp_raw()#enquanto não tiver reconhecendo, vai tentar ler
    equals_pos = lines[1].find('t=') # t= temperatura (mas precisa /1000 para achar a real)
    if equals_pos != -1:
...
while True:
    ...
    try:
        ...
        seconds += 1  # aux leitura 5 em 5 seg
        sec +=1  # aux gravação bcd 1->2x por min

    #Se tiver mais de 1 leitura, ele calcula a média corretamente
    ...
    #Pelo tamanho do código, ele vai demorar para ler, por isso 2(mas são 5 seg)
        if (seconds == 2):

          # Maior X Menor Temp/Umi
          ...
          #Escreve arq. sec
            ...
            try:
                if (sec == 12 ): 
                  ...
                  # Para saber qual field precisa mudar, faça uma consulta na query
        ...
        if hora in hours: # Compara tempo atual com horários estabelecidos antes
          # Faz a média de hora/hora
